...
export default function App() {
  ...
  // Request Storage Persistence on mount for mobile/tablet browsers to avoid auto-eviction
  useEffect(() => {
    if (navigator.storage && navigator.storage.persist) {
      navigator.storage.persist().then((persistent) => {
        if (persistent) {
          console.log("🔒 Storage persistence granted. Browser will protect this client data from OS eviction.");
        } else {
          console.warn("⚠️ Storage persistence not fully guaranteed. UA might clear storage under high pressure.");
        }
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (savedCasting) {
      ...
      setCastingRecords(parsed);
    }
    if (savedTreeCasting) {
      const parsed = JSON.parse(savedTreeCasting);
      setTreeCastingRecords(parsed);
    }
    if (savedRolling) {
      try {
